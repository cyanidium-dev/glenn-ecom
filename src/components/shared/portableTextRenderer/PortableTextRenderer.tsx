...
const components: Partial<PortableTextReactComponents> = {
  block: {
    normal: ({ children }) => {
      ...
      return (
        <p className="text-[12px] leading-[116%] lg:text-[14px] lg:leading-[121%]">{children}</p>
      );
    },
  },
  marks: {
    ...
    number: (props) => {
      ...
      if (props.children) {
        ...
        if (isValidElement(firstChild)) {
          ...
          const childValue = (firstChild.props as { value?: CustomPortableTextBlock })?.value;
  ...
  listItem: {
    bullet: ({ children }) => <li className="text-[12px] leading-[116%] lg:text-[14px] lg:leading-[121%]">{children}</li>,
    number: ({ children }) => <li className="text-[12px] leading-[116%] lg:text-[14px] lg:leading-[121%]">{children}</li>,
  },
};
